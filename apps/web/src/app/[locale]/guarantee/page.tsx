    ...
    <div className="max-w-3xl mx-auto px-5 py-12">
      <div className="text-center mb-10">
        ...
        <div className="text-3xl font-black text-teal-300 mb-3">
          ...
          <span className="text-sm text-white/50 font-semibold"> {locale === "tr" ? "'ye kadar koruma" : "coverage"}</span>
        </div>
        <p className="text-white/60 max-w-xl mx-auto">{t("guarantee.subtitle")}</p>
      </div>

      <h2 className="text-xl font-bold mb-4">{t("guarantee.howTitle")}</h2>
      <div className="grid sm:grid-cols-2 gap-4 mb-10">
        {POINTS.map((p) => (
          <div key={p.k} className="bg-ink-card border border-white/10 rounded-2xl p-5">
            <div className="text-3xl mb-3">{p.icon}</div>
            <h3 className="font-bold mb-1.5">{t(`guarantee.${p.k}title` as any)}</h3>
            <p className="text-white/60 text-sm">{t(`guarantee.${p.k}desc` as any)}</p>
          </div>
        ))}
      </div>
